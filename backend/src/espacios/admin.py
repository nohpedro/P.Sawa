...
from .models import TipoActividad, Espacio, EspacioActividad, Calendario, Regla, ReglaGlobal, Promocion
...
class EspacioAdmin(admin.ModelAdmin):
    list_display = ("nombre", "estado", "capacidad", "ubicacion", "created_at")
...
class ReglaAdmin(admin.ModelAdmin):
    list_display = ("espacio", "frecuencia", "intervalo", "weekday_mask", "hora_inicio", "hora_fin", "fecha_desde", "fecha_hasta", "activo")
...
class ReglaGlobalAdmin(admin.ModelAdmin):
    list_display = ("nombre", "frecuencia", "intervalo", "weekday_mask", "hora_inicio", "hora_fin",
                    "fecha_desde", "fecha_hasta", "aplica_todos", "activo")
    list_filter = ("frecuencia", "aplica_todos", "activo")
    search_fields = ("nombre", "descripcion")
    filter_horizontal = ("espacios",)  # para elegir múltiples espacios

    def save_model(self, request, obj, form, change):
        # Si no aplica a todos, recomendamos que haya al menos un espacio
        super().save_model(request, obj, form, change)
        if not obj.aplica_todos and obj.espacios.count() == 0:
            self.message_user(request, "Sugerencia: Selecciona al menos un espacio si 'aplica_todos' es False.")
