...
export default function BusinessGoalDetailPage() {
  ...
  return (
    <div style={{ display: "grid", gap: 18 }}>
      ...
      {goal && (
        <>
          <Card title={goal.nombre} subtitle="Detalle, progreso y acciones del ciclo actual." rightSlot={<div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Link to={PATHS.businessGoalNodes.replace(":id", goal.id)}><Button>Asignar variables</Button></Link><Button onClick={() => setModal("contribution")}>+ Movimiento</Button><Button variant="outline" onClick={() => setModal("details")}>Detalles</Button><Button variant="outline" onClick={() => setModal("renew")}>Renovar</Button><Link to={PATHS.businessGoalEdit.replace(":id", goal.id)}><Button variant="outline">Editar</Button></Link><Button variant="outline" onClick={() => navigate(PATHS.businessGoalNodes.replace(":id", goal.id))}>Nodos</Button><Button variant="danger" onClick={() => setModal("delete")}>Eliminar</Button></div>}>
